...
class FormManager {
    ...
    inicializarEventos() {
        ...
        this.linkVoltarLogin?.addEventListener('click', (e) => this.voltarParaLogin(e));
        this.linkEsqueciSenha?.addEventListener('click', (e) => this.mostrarRecuperacaoSenha(e));
        ...
        // Tema
        this.btnToggleDarkMode?.addEventListener('click', () => this.alternarTema());
        
        // Cadastro
        document.querySelector('#btnCadastrar')?.addEventListener('click', (e) => this.realizarCadastro(e));
        
        // Redefinição de Senha
        document.querySelector('#btnRedefinirSenha')?.addEventListener('click', () => this.redefinirSenha());
    ...
    inicializarValidacoes() {
        ...
        confirmeSenhaCadastro?.addEventListener('input', () => this.verificarForcaSenha());
    ...
    inicializarCamposCondicionais() {
        ...
        participaMinisterio?.addEventListener('change', () => {
            if (selectMinisterio) {
                participaMinisterio.checked ? mostrarElemento(selectMinisterio) : ocultarElemento(selectMinisterio);
            }
        });

        participaCelula?.addEventListener('change', () => {
            if (selectCelula) {
                participaCelula.checked ? mostrarElemento(selectCelula) : ocultarElemento(selectCelula);
            }
        });
    }
    ...
    verificarTema() {
        const isDarkMode = localStorage.getItem(storageManager.KEYS.TEMA) === "true";
    ...
    async realizarLogin() {
        ...
        try {
            ...
            this.loginSucesso(data.usuario.tipoUsuario, data.usuario.id, data.usuario.idCelula);
        } catch (error) {
    ...
    async realizarCadastro(e) {
        ...
        try {
            ...
            mostrarToast('Erro ao realizar cadastro. Por favor, tente novamente.', 'error');
        } finally {
            this.ocultarLoading();
        }
    }
    ...
    validarFormularioCadastro() {
        ...
        if (camposVazios.length > 0) {
            mostrarToast(`Por favor, preencha os campos: ${camposVazios.join(', ')}`, 'error');
            return false;
        }
    ...
    validarSenha() {
        ...
        const confirmeSenha = document.querySelector('#confirmeSenhaCadastro').value;
        ...
        if (!/(?=.*[A-Z])/.test(senha)) {
            this.marcarErroSenha('A senha deve conter pelo menos uma letra maiúscula');
            return false;
        }
        ...
        if (!/(?=.*[!@#$%^&*])/.test(senha)) {
            this.marcarErroSenha('A senha deve conter pelo menos um caractere especial (!@#$%^&*)');
            return false;
        }

        if (senha !== confirmeSenha) {
            document.querySelector('#senhaCadastro').classList.add('input-error');
            document.querySelector('#confirmeSenhaCadastro').classList.add('input-error');
    ...
    coletarDadosCadastro() {
        ...
        const checkboxes = {
            concluiuBatismo: document.querySelector('#concluiuBatismo')?.checked || false,
            participouCafe: document.querySelector('#participouCafe')?.checked || false,
            participaMinisterio: document.querySelector('#participaMinisterio')?.checked || false,
            participaCelula: document.querySelector('#participaCelula')?.checked || false
        };

        const selects = {
            nomeMinisterio: document.querySelector('#selectMinisterio')?.value?.trim() || '',
            idCelula: document.querySelector('#selectCelula')?.value?.trim() || '',
            tipoUsuario: 'UsuarioComum'
        };

        const cursos = [
            'cursoMeuNovoCaminho', 'cursoVidaDevocional', 'cursoFamiliaCrista',
            'cursoVidaProsperidade', 'cursoPrincipiosAutoridade', 'cursoVidaEspirito',
            'cursoCaraterCristo', 'cursoIdentidadesRestauradas'
        ].reduce((acc, curso) => ({
            ...acc,
    ...
    inicializarEventosRecuperacao() {
        document.querySelector('#botaoRecuperar')
            ?.addEventListener('click', () => this.enviarEmailRecuperacao());
            
        document.querySelector('#botaoFechar')
            ?.addEventListener('click', () => this.fecharModalRecuperacao());
            
        document.querySelector('#formRecuperacaoSenha')
        ...
        document.querySelector('#emailRecuperacao')
    ...
    async enviarEmailRecuperacao() {
        ...
        try {
            ...
            mostrarToast('Erro ao enviar e-mail. Tente novamente mais tarde.', 'error');
        } finally {
            this.ocultarLoading();
        }
    }
    ...
    verificarForcaSenha() {
        ...
        this.atualizarIndicadorForcaSenha(forca, forcaSenha, forcaSenhaTexto, senha, confirmeSenha);
    }
    ...
    atualizarIndicadorForcaSenha(forca, elemento, texto, senha, confirmeSenha) {
        ...
        for (const [limite, config] of Object.entries(forcas).sort((a, b) => b[0] - a[0])) {
    ...
    atualizarClassesForcaSenha(elemento, tipo) {
        elemento.classList.remove('progress-success', 'progress-warning', 'progress-error');
