import { mostrarToast, mostrarElemento, ocultarElemento, salvarEstadoFormulario, getEstadoFormulario } from './utils.js';
...
class DashboardManager {
    ...
    async carregarDashboard(tipoUsuario, idUsuario, idCelula) {
        try {
            ...
            if (this.dashboardContent) {
                this.dashboardContent.innerHTML = this.gerarLoadingHTML(mensagemCarregamento);
            }
    ...
    mostrarMensagemResultado(dados) {
        if (Array.isArray(dados)) {
            if (dados.length === 0) {
                ...
                mostrarToast(`${dados.length} membro(s) encontrado(s) em sua célula`, 'success');
            }
        }
    }
    ...
    gerarConteudoAdmin(dados) {
        ...
        <div class="flex flex-col items-center w-full">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 w-full">
                ${dados.filter(usuario => usuario.email !== 'admin@casadopai.com').map(usuario => this.gerarCardUsuario(usuario)).join('')}
            </div>
        </div>
    `;
}
    ...
    gerarConteudoLider(dados) {
        ...
        return `
            <div class="flex flex-col items-center w-full">
                <h2 class="text-2xl font-bold mb-4 text-center">Painel do Líder de Célula</h2>
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 w-full">
                    ${dados.map(usuario => this.gerarCardUsuario(usuario)).join('')}
                </div>
            </div>
        `;
    }
        ...
        return `
            <div class="card bg-base-100 shadow-xl user-card">
                <div class="card-body">
                    <h3 class="card-title">${usuario.nomeCompleto}</h3>
                    <p>Email: ${usuario.email}</p>
                    <p>Tipo: ${this.formatarTipoUsuario(usuario.tipoUsuario)}</p>
    ...
    adicionarEventosDashboard() {
        ...
        btnDetalhes.forEach(btn => {
            btn.addEventListener('click', () => this.mostrarDetalhesUsuario(btn.dataset.id));
        });
    }
    ...
    async mostrarDetalhesUsuario(id) {
        ...
        try {
            this.mostrarLoading();
            const response = await this.fetchComTimeout(`http://localhost:3000/usuarios/${id}`);
            ...
            mostrarToast(error.message || 'Erro ao carregar detalhes do usuário', 'error');
        } finally {
            this.ocultarLoading();
        }
    }
    ...
    preencherCamposUsuario(usuario) {
        const campos = {
            ...
            '#detalhesDataNascimento': usuario.dataNascimento ? new Date(usuario.dataNascimento).toLocaleDateString() : 'Não disponível',
            ...
            '#detalhesParticipaMinisterio': usuario.participaMinisterio ? 'Sim' : 'Não',
    ...
    adicionarBotoesAcao(usuario) {
        ...
        if (this.tipoUsuarioAtual === 'Administrador' || 
           (this.tipoUsuarioAtual === 'LiderCelula' && usuario.tipoUsuario !== 'Administrador')) {
            let botoesHTML = `
    ...
    async editarUsuario(id) {
        ...
        try {
            ...
            mostrarToast(error.message || 'Erro ao carregar dados para edição', 'error');
        } finally {
            this.ocultarLoading();
        }
    }
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div class="form-control">
                        ...
                        <input type="date" id="editDataNascimento" class="input input-bordered" 
                            value="${new Date(usuario.dataNascimento).toISOString().split('T')[0]}" required
                            onchange="this.classList.remove('input-error')">
                    </div>
                    ...
                    <div class="form-control">
                        ...
                        <select id="editMinisterio" class="select select-bordered">
                            <option value="" ${usuario.nomeMinisterio ? '' : 'selected'}>Não participa</option>
                            <option value="Acolhimento" ${usuario.nomeMinisterio === 'Acolhimento' ? 'selected' : ''}>Acolhimento</option>
                            <option value="Mídias" ${usuario.nomeMinisterio === 'Mídias' ? 'selected' : ''}>Mídias</option>
                            <option value="Louvor" ${usuario.nomeMinisterio === 'Louvor' ? 'selected' : ''}>Louvor</option>
                            <option value="Dança" ${usuario.nomeMinisterio === 'Dança' ? 'selected' : ''}>Dança</option>
                            <option value="Infantil" ${usuario.nomeMinisterio === 'Infantil' ? 'selected' : ''}>Infantil</option>
                            <option value="Teens" ${usuario.nomeMinisterio === 'Teens' ? 'selected' : ''}>Teens</option>
                        </select>
                    </div>
                    ...
                    <div class="form-control">
                        ...
                        <select id="editCelula" class="select select-bordered">
                            <option value="" ${usuario.idCelula ? '' : 'selected'}>Não participa</option>
                    ...
                    <div class="form-control">
                        <label class="label cursor-pointer">
                            ...
                            <input type="checkbox" id="editConcluiuBatismo" class="checkbox" ${usuario.concluiuBatismo ? 'checked' : ''}>
                        </label>
                    </div>
                </div>
    ...
    gerarOpcoesCelulas(idCelulaSelecionada) {
        return `
            <option value="1" ${idCelulaSelecionada === 1 ? 'selected' : ''}>Aba Pai</option>
            <option value="2" ${idCelulaSelecionada === 2 ? 'selected' : ''}>Dunamis</option>
            <option value="3" ${idCelulaSelecionada === 3 ? 'selected' : ''}>Exousia</option>
            <option value="4" ${idCelulaSelecionada === 4 ? 'selected' : ''}>Koinonia</option>
            <option value="5" ${idCelulaSelecionada === 5 ? 'selected' : ''}>Luz do Mundo</option>
            <option value="6" ${idCelulaSelecionada === 6 ? 'selected' : ''}>Zoe</option>
            <option value="7" ${idCelulaSelecionada === 7 ? 'selected' : ''}>A Rede</option>
            <option value="8" ${idCelulaSelecionada === 8 ? 'selected' : ''}>Atos 2</option>
            <option value="9" ${idCelulaSelecionada === 9 ? 'selected' : ''}>El Shamah</option>
            <option value="10" ${idCelulaSelecionada === 10 ? 'selected' : ''}>Freedom</option>
            <option value="11" ${idCelulaSelecionada === 11 ? 'selected' : ''}>Hermon</option>
            <option value="12" ${idCelulaSelecionada === 12 ? 'selected' : ''}>Maranata</option>
            <option value="13" ${idCelulaSelecionada === 13 ? 'selected' : ''}>Reobote</option>
            <option value="14" ${idCelulaSelecionada === 14 ? 'selected' : ''}>Sal da Terra</option>
        `;
    }    
    ...
    async deletarUsuario(id) {
        ...
        try {
            this.mostrarLoading();
            await this.fetchComTimeout(`http://localhost:3000/usuarios/${id}`, { 
                method: 'DELETE',
                headers: {
                    'Content-Type': 'application/json'
                }
    ...
    async tornarLider(id) {
        ...
        try {
            ...
            const verificacaoResponse = await this.fetchComTimeout(`http://localhost:3000/usuarios/${id}`);
            const usuario = await verificacaoResponse.json();

            if (!usuario.idCelula) {
                mostrarToast('O usuário precisa estar associado a uma célula para se tornar líder', 'error');
                return;
            }
            ...
            const response = await this.fetchComTimeout(`http://localhost:3000/usuarios/${id}/tornar-lider`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                }
            ...
            await this.atualizarInterfaceAposAcao('Usuário promovido a líder de célula com sucesso');
        } catch (error) {
            console.error('Erro ao promover usuário:', error);
            mostrarToast(error.message || 'Erro ao promover usuário a líder', 'error');
        } finally {
    ...
    async rebaixarLider(id) {
        ...
        if (!confirm('Tem certeza que deseja rebaixar este líder para usuário comum? Esta ação afetará todos os membros da célula.')) {
            return;
        }
        ...
        try {
            ...
            const response = await this.fetchComTimeout(`http://localhost:3000/usuarios/${id}/rebaixar-lider`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                }
            ...
            setTimeout(() => {
                this.fecharModal();
                this.carregarDashboard(this.tipoUsuarioAtual, this.idUsuarioAtual, this.idCelulaAtual);
            }, 1500);
    ...
    async atualizarInterfaceAposAcao(mensagem, tipo = 'success', delay = 1500) {
        ...
        this.fecharModal();
        await this.carregarDashboard(this.tipoUsuarioAtual, this.idUsuarioAtual, this.idCelulaAtual);
    }
